...
function withInjectedCredentials(
  payload,
  { username, password, customerNo, labelType, customerSubjectName }
) {
  ...
  if (
    customerSubjectName &&
    !hasKeyInsensitive(enriched, "customerSubjectName")
  ) {
    enriched.customerSubjectName = customerSubjectName;
  }

  const subjectName = getValueCaseInsensitive(enriched, "customerSubjectName");
  if (typeof subjectName !== "string" || !subjectName.trim()) {
    delete enriched.customerSubjectName;
  }

  return enriched;
}

function sanitizeLabelLayoutFields(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return payload;
  }

  const normalizedLabelType = String(getValueCaseInsensitive(payload, "labelType") || "").toLowerCase();
  ...
  delete sanitized.labelA4StartPosition;
  delete sanitized.labelA4MoveXMm;
  delete sanitized.labelA4MoveYMm;
  return sanitized;
}
...
export async function createGlsLabel({
  ...
  const customerSubjectName = process.env.GLS_CUSTOMER_SUBJECT_NAME;
  ...
  const enrichedPayload = withInjectedCredentials(payload, {
    username,
    password,
    customerNo,
    labelType,
  ...
  const finalPayload = sanitizeLabelLayoutFields(enrichedPayload);
